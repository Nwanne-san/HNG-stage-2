...
const ProductSection = () => {
  ...
  return (
    <div className="w-full" id="our-products">
      <Toaster />
      <div className="flex flex-row justify-around w-full gap-[104px] px-4 sm:px-8 py-[4em] ">
        <div className="hidden sm:grid justify-center lg:grid-cols-4 sm:grid-cols-3 gap-8">
          {currentProducts.map((product) => (
            <div
              key={product.id}
              className="flex flex-col items-center bg-beige w-full h-fit rounded-2xl "
            >
              ...
              <div className="flex flex-col justify-center items-center gap-4 bg-white py-4 w-full px-2 rounded-b-2xl ">
                <div className="flex flex-col justify-center items-center gap-1">
                  <p
                    className={`${playfair.className} text-pink font-medium text-[20px] leading-[28px]`}
                  >
                    {product.name}
                  </p>
                  <p
                    className={`${lato.className} text-[18px] leading-[21px] flex gap-[3px]`}
                  >
                    <Image src={Currency} />
                    {product.price}
                  </p>
        ...
        <div className="carousel-container grid w-full sm:gap-6 gap-4 sm:hidden">
          {currentProducts.map((product) => (
            <div
              key={product.id}
              className="flex flex-col carousel-item items-center w-full h-fit rounded-2xl "
            >
              <div className="relative">
                <Image
                  src={Like}
